...
def INICOND_convdiff_blob(xyz, domain_size=[1, 1, 1], blob_pos=[0.75, 0.5, 0], blob_width=0.01):
    x = xyz[0] - blob_pos[0]
    y = xyz[1] - blob_pos[1]
    ...
    for i_x in [-1, 0, 1]:
        for i_y in [-1, 0, 1]:
            ...
            for i_z in r_z:
                out += np.exp( -( (x + i_x*domain_size[0])**2 + (y + i_y*domain_size[1])**2 + (z + i_z*domain_size[2])**2 ) / blob_width )
    return out