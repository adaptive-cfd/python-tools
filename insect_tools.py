...
def load_t_file( fname, interp=False, time_out=None, return_header=False,
                verbose=True, T0=None, keep_duplicates=False, 
    ...
    ------
         ...
         optimized_loading: bool
             If true, then we read the actual ASCII *.t file ideally only once, because this can be 
             quite slow. We read it, then store the raw data array in an compressed, binary *.npz file. On the next read,
             this file is read rather than the ascii file, and this is much faster. If the modification time stamp of the *.t
             file is newer than the *.npz file, we re-read the file from *.t (and store it again to *.npz)
    ...
    # RAW data reading
    #--------------------------------------------------------------------------
    # get size of the t file from filesystem
    size_t = os.path.getsize(fname)
    # variable to read NPZ data to
    Q = None
    ...
    if not os.path.isfile(fname+'.npz'):
        ...
        # file exists 
        date1 = datetime.datetime.fromtimestamp(os.path.getmtime(fname+'.npz')) 
        date2 = datetime.datetime.fromtimestamp(os.path.getmtime(fname))
        
        if date1 < date2:
            # *.npz file is older than source *.t file - reloading is required
            reloading_required = True
            
        # second condition: if the NPZ file appears valid, then check if the filesize is the same
        # This catches goofy errors such as rsync with different time stamp etc that could block us from
        # re-reading the file.
        if not reloading_required:
            try:
                Q = np.load(fname+'.npz')
    ...
    if not optimized_loading or reloading_required:
        ...
        if optimized_loading:
            ...
            np.savez(fname+'.npz', data_raw=data_raw, size_t=size_t)
    ...
    #-----------------------------------------------
        ...
        # re-load the file (as we may have updated it)
        Q = np.load(fname+'.npz')
        data_raw = Q['data_raw']
...
def get_wing_membrane_grid(fname, dx=1e-3, dy=1e-3, return_1D_list=True):
    ...
    if return_1D_list:
        ...
        # 2D mode
        X, Y = np.meshgrid(x_grid, y_grid) # 1st index: y, 2nd index:x
        X = X.T 
        Y = Y.T # 1st index: x, 2nd index:y
        
        mask = np.zeros_like(X)
        
        for ix in range(X.shape[0]):
            for iy in range(X.shape[1]):
                if polygon.contains( Point(X[ix,iy], Y[ix,iy]) ):
                    mask[ix,iy] = 1.0
